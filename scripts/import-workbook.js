...
async function getWeekId(isoDate, serial = null) {
  if (!isoDate) return null;
  const found = await one("SELECT id FROM weeks WHERE week_start = ?", [isoDate]);
...
async function findIngredientId(name, extra = {}) {
...
async function importProductionPlan(wb) {
  ...
  for (let col = 2; col <= range.e.c + 1; col += 1) {
    ...
    if (iso) {
      weekCols.push({ col, serial, weekId: await getWeekId(iso, serial), iso });
    }
  }
  ...
  for (let row = 6; row <= 38; row += 1) {
    ...
    if (!name) continue;
    const productId = await findProductId(name, {
...
async function importIngredientPlan(wb) {
  ...
  for (let row = 1; row <= range.e.r + 1; row += 1) {
    ...
    const ingredientId = await findIngredientId(name, {
    ...
    for (let col = 4; col <= range.e.c + 1; col += 1) {
      ...
      const iso = excelDate(serial);
      if (iso) weekCols.push({ col, weekId: await getWeekId(iso, serial) });
    }
    ...
    for (let productRow = row + 8; productRow < endingRow; productRow += 1) {
      ...
      const productId = await findProductId(productName);
      await run(
...
async function importVelocity(wb) {
  ...
  for (let row = 2; row <= range.e.r + 1; row += 1) {
    ...
    const productId = await findProductId(itemName, {
...
async function importPurchaseCalendar(wb) {
  ...
  for (let row = 2; row <= range.e.r + 1; row += 2) {
    ...
    const weekId = await getWeekId(excelDate(serial), serial);
    if (!weekId) continue;
    for (let col = 3; col <= range.e.c + 1; col += 1) {
      ...
      const ingredientId = item ? await findIngredientId(item) : null;
      await run(
...
async function importFormulaTabs(wb) {
  const skip = new Set([
    "Purchase Calander",
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    const productId = productName ? await findProductId(productName, { source_sheet: sheetName }) : null;
...
export async function importWorkbook(filePath) {
  ...
  const production = await importProductionPlan(wb);
  const ingredients = await importIngredientPlan(wb);
  const velocity = await importVelocity(wb);
  const purchaseCalendar = await importPurchaseCalendar(wb);
  const formulaLines = await importFormulaTabs(wb);
  const recommendations = (await regenerateRecommendations()).length;
  return {
